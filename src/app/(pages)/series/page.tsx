...
export default function SeriesPage() {
    ...
    const loadInitialSeries = async (): Promise<void> => {
        setLoading(true);
        setError(null);
        setPage(1);
        setSeries([]);
        ...
        try {
            ...
            // Son 3 yılın dizilerini yükle
            for (let y = currentYear; y >= currentYear - 2 && allSeries.length < 20; y--) {
                try {
                    ...
                    logError("Error fetching series", err, { year: y });
                }
            }
            ...
            setSeries(allSeries);
            setTotalResults(allSeries.length);
            setHasMore(allSeries.length >= 10);
            setCurrentYear(currentYear);
        } catch (err) {
            setError("Diziler yüklenirken bir hata oluştu.");
        } finally {
            setLoading(false);
            ...
            if (query && query.length >= 3) {
                ...
                setPage(prevPage => {
                    ...
                    fetchMovies({
                        ...
                        if (seriesData.length === 0) {
                            ...
                            setTotalResults(prev => prev + seriesData.length);
                        }
                ...
                setPage(prevPage => {
                    ...
                    const attemptLoad = async () => {
                        for (let attempt = 0; attempt < 5 && !foundSeries; attempt++) {
                            ...
                            try {
                                ...
                                if (data.Search && data.Search.length > 0) {
                                    setSeries(prev => {
                                        ...
                                        const uniqueSeries = searchResults.filter((s: Movie) => !fetchedIds.has(s.imdbID));
                                        
                                        if (uniqueSeries.length > 0) {
                                            foundSeries = true;
                                            setTotalResults(totalPrev => totalPrev + uniqueSeries.length);
                                            setLoadingMore(false);
                                ...
                                logError("Error loading more series", err);
                            }
                        }
    ...
    const handleFetchSeries = async (reset: boolean = false): Promise<void> => {
        ...
        try {
            ...
            if (reset) {
                ...
                setHasMore(seriesData.length === 10 && total > 10);
            } else {
                setSeries(prev => [...prev, ...seriesData]);
                setTotalResults(prev => prev + seriesData.length);
                setHasMore(seriesData.length === 10 && total > (currentPage * 10));
            }

            setPage(currentPage + 1);
        } catch (err) {
            setError(err instanceof Error ? err.message : "Diziler yüklenirken bir hata oluştu.");
            if (reset) {
                setSeries([]);
                setTotalResults(0);
            }
    ...
    // Infinite scroll için en güncel dizileri yükle
    useEffect(() => {
        if (!query && !year) {
            loadInitialSeries();
        }
    ...
    }, [hasMore, loadingMore, loading, loadMoreSeries]);
            ...
            <div className="relative w-full px-2 sm:px-3 md:px-4 lg:px-6 xl:px-8 py-4 sm:py-6 md:py-8 lg:py-10">
                ...
                {loading && isInitialLoad.current && (
                    <div className="flex justify-center my-8 sm:my-10 md:my-12">
                        <LoadingSpinner message="Diziler yükleniyor..." />
                    </div>
                )}
                ...
                {/* Empty State */}
                {!loading && !error && series.length === 0 && query.length >= 3 && (
                ...
                {/* Initial State */}
                {!loading && !error && series.length === 0 && query.length < 3 && !year && (
                    <div className="text-center py-8 sm:py-12 md:py-16">
                        <p className="text-gray-400 text-xs sm:text-sm md:text-base lg:text-lg px-4">
                            En güncel diziler yükleniyor... Aşağı kaydırarak daha fazlasını görebilirsiniz.
                        </p>
                    </div>
                )}
            </div>
